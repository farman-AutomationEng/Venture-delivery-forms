...
function populateInvoice(d) {
  ...
  sv("id",  d.date); sv("ia", d.acctNo); sv("ino", d.invoiceNo);
  sv("ibt", d.billTo); sv("ist", d.shipTo); sv("ipo", d.po);
  sv("itr", d.terms); sv("iso", d.salesOrder); sv("ism", d.shippingMethod);
  sv("ish", d.shipping);

  // Update display elements (new PDF-style layout)
  function sd(id, val){ var e=document.getElementById(id); if(e) e.textContent=val||"—"; }
  sd("ia-d",  d.acctNo);
  sd("id-d",  d.date);
  sd("ino-d", d.invoiceNo);
  sd("ibt-d", d.billTo);
  sd("ist-d", d.shipTo);
  sd("ipo-d", d.po);
  sd("itr-d", d.terms);
  sd("iso-d", d.salesOrder);
  sd("ism-d", d.shippingMethod);

  var ship = parseFloat(d.shipping||"0");
  sd("ish-d",  ship>0 ? "$"+ship.toFixed(2) : "—");
  sd("ish-val", ship>0 ? "$"+ship.toFixed(2) : "$0.00");

  // Totals
  var items = d.lineItems || [];
  var sub = items.reduce(function(s,li){ return s+(parseFloat(li.total)||0); }, 0);
  sd("itot-sub", "$"+sub.toFixed(2));
  var due = sub + ship;
  sd("itot", d.grandTotal || ("$"+due.toFixed(2)));

  // Line items display
  var itbd = document.getElementById("itb-d");
  if (itbd) {
    while(itbd.firstChild) itbd.removeChild(itbd.firstChild);
    if(!items.length){
      var empty = document.createElement("div");
      empty.style.cssText = "padding:16px;text-align:center;color:#94a3b8;font-size:12px;font-style:italic";
      empty.textContent = "No line items";
      itbd.appendChild(empty);
    }
    items.forEach(function(li, idx) {
      var row = document.createElement("div");
      row.style.cssText = "display:grid;grid-template-columns:80px 40px 1fr 80px 80px;font-size:12px;border-bottom:1px solid #F3F4F6;"+(idx%2===1?"background:#F9FAFB":"");
      var cells = [
        {v:li.item||"—",    s:"padding:8px 10px;font-weight:600"},
        {v:li.qty||"1",     s:"padding:8px 6px;text-align:center"},
        {v:li.description||"—", s:"padding:8px 10px;color:#374151"},
        {v:li.unitPrice?"$"+parseFloat(li.unitPrice).toFixed(2):"—", s:"padding:8px 10px;text-align:right"},
        {v:li.total?"$"+parseFloat(li.total).toFixed(2):"—", s:"padding:8px 10px;text-align:right;font-weight:700;color:#00263E"}
      ];
      cells.forEach(function(c){
        var d2=document.createElement("div"); d2.style.cssText=c.s; d2.textContent=c.v; row.appendChild(d2);
      });
      itbd.appendChild(row);
    });
  }
}
